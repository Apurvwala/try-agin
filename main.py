...
flask_app = Flask(__name__) # Renamed to avoid conflict with Kivy App class
...
@flask_app.route('/get_user_email', methods=['POST'])
...
@flask_app.route('/send_otp', methods=['POST'])
...
@flask_app.route('/verify_otp', methods=['POST'])
...
@flask_app.route('/start_update_capture', methods=['POST'])
...
@flask_app.route('/get_last_recognized', methods=['GET'])
...
class FlaskThread(threading.Thread):
    def __init__(self, app_instance):
        ...
        self.daemon = True # Allow the thread to exit when the main program exits
        self.running = False

    def run(self):
        self.running = True
        KivyLogger.info("[INFO] Flask server thread starting...")
        try:
            # Host on 0.0.0.0 to be accessible within the Android device's network (localhost for the app)
            # Use a specific port, e.g., 5000
            self.app.run(host='0.0.0.0', port=5000, debug=False, use_reloader=False)
        except Exception as e:
            KivyLogger.error(f"[ERROR] Flask server crashed: {e}")
        finally:
            self.running = False
            KivyLogger.info("[INFO] Flask server thread stopped.")

    def stop(self):
        # This is a soft stop. Flask's development server doesn't have a direct stop method.
        # For production, you'd use a WSGI server like Gunicorn/Waitress with proper shutdown.
        # For a simple Kivy app, exiting the main Kivy app will usually terminate this daemon thread.
        # If you need a more robust shutdown, you'd implement a custom Flask command.
        KivyLogger.info("[INFO] Attempting to stop Flask server thread (may not be immediate)...")
        self.running = False # Signal to stop, though Flask's run() is blocking.

# --- Kivy App ---
class FaceAppKivy(App):
    flask_thread = None
    ...
    def build(self):
        ...
        self.server_status_label = Label(text="Flask Server: Not Running", size_hint_y=0.2)
        layout.add_widget(self.server_status_label)

        start_button = Button(text="Start Flask Server", size_hint_y=0.4)
        start_button.bind(on_press=self.start_flask_server)
        layout.add_widget(start_button)

        stop_button = Button(text="Stop Flask Server", size_hint_y=0.4)
        stop_button.bind(on_press=self.stop_flask_server)
        layout.add_widget(stop_button)

        return layout

    def start_flask_server(self, instance):
        if self.flask_thread is None or not self.flask_thread.is_alive():
            KivyLogger.info("[INFO] Starting Flask server...")
            self.flask_thread = FlaskThread(flask_app)
            self.flask_thread.start()
            self.update_status_label("Flask Server: Running (Port 5000)")
        else:
            KivyLogger.warning("[WARN] Flask server is already running.")
            self.update_status_label("Flask Server: Already Running")

    def stop_flask_server(self, instance):
        if self.flask_thread and self.flask_thread.is_alive():
            KivyLogger.info("[INFO] Stopping Flask server...")
            self.flask_thread.stop()
            # Give a moment for the thread to potentially clean up, then update UI
            Clock.schedule_once(lambda dt: self.update_status_label("Flask Server: Stopped"), 1)
        else:
            KivyLogger.warning("[WARN] Flask server is not running.")
            self.update_status_label("Flask Server: Not Running")

    def update_status_label(self, text):
        # Schedule UI update on the main thread
        Clock.schedule_once(lambda dt: self._update_label_text(text), 0)

    def _update_label_text(self, text):
        if self.server_status_label:
            self.server_status_label.text = text

    def on_stop(self):
        # Ensure Flask server is stopped when Kivy app closes
        KivyLogger.info("[INFO] Kivy app stopping. Attempting to stop Flask server.")
        if self.flask_thread and self.flask_thread.is_alive():
            self.flask_thread.stop()
...
if __name__ == '__main__':
    # Ensure the 'known_faces' directory exists before starting the app
